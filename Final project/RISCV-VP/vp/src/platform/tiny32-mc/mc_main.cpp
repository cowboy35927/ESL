...
public:
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > Filter_1_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	SobelFilter Filter_0("Filter_0");
	SobelFilter Filter_1("Filter_1");
	...
	iss0_mem_if.bus_lock = bus_lock;
	iss1_mem_if.bus_lock = bus_lock;

	instr_memory_if *instr0_mem_if = &iss0_mem_if;
	data_memory_if *data0_mem_if = &iss0_mem_if;

	instr_memory_if *instr1_mem_if = &iss1_mem_if;
	data_memory_if *data1_mem_if = &iss1_mem_if;

	if (opt.use_instr_dmi) {
		instr0_mem_if = &instr_mem0;
		instr1_mem_if = &instr_mem1;
	}
	if (opt.use_data_dmi) {
		iss0_mem_if.dmi_ranges.emplace_back(dmi);
		iss1_mem_if.dmi_ranges.emplace_back(dmi);
	}
	...
	dma_connector.isock.bind(bus.tsocks[2]);

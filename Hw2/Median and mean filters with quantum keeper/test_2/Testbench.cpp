        ...
        for (int count_x = -2; count_x < 3; count_x++)
        {
          ...
          { //-1, 0, 1
            ...
            { //-1, 0, 1
              ...
              //cout << "Now at " << sc_time_stamp() << " TB " <<delay<< endl; // print current sc_time
            }
          }
          if (count_y == 2 && count_x == 0)
          {
            ...
            while(!done){
              m_qk.inc( delay );
              initiator.read_from_socket(SOBEL_FILTER_CHECK_ADDR, mask, data.uc, 4 ,delay);
              output_num = data.sint;
              if (m_qk.need_sync()) m_qk.sync();
              if(output_num>0) done=true;
            }
            //wait(10 * CLOCK_PERIOD, SC_NS);
            initiator.read_from_socket(SOBEL_FILTER_RESULT_ADDR, mask, data.uc, 4 ,delay);
            //cout << "Now at " << sc_time_stamp() << " R: "<< data.uint/(256*256)<<",delay:"<<delay<< endl; // print current sc_time
            //cout << "Now at " << sc_time_stamp() << " G: "<<(data.uint/(256))%256<<",delay:"<<delay<< endl; // print current sc_time
            //cout << "Now at " << sc_time_stamp() << " B: "<< (data.uint)%(256)  <<",delay:"<<delay<< endl; // print current sc_time    
            //total = data.sint_b;

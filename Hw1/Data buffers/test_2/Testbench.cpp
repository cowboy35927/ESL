...
void Testbench::do_sobel()
{
  ...
  for (y = 0; y != height; ++y)
  {
    ...
    for (x = 2; x < width; x = x + 3)
    {
      ...
      for (int count_y = 0; count_y < 3; count_y++)
      {
        for (int count_x = -2; count_x < 3; count_x++)
        {
          ...
          if (count_y == 2 && count_x == 0)
          {
            ...
            *(target_bitmap + bytes_per_pixel * (width * y + x - 2) + 2) = i_result_r.read();
            *(target_bitmap + bytes_per_pixel * (width * y + x - 2) + 1) = i_result_g.read();
            *(target_bitmap + bytes_per_pixel * (width * y + x - 2) + 0) = i_result_b.read();
            wait(1);
            count += 1;
            // cout << "Now at " << sc_time_stamp() << " TB454456465465 " << count << endl; // print current sc_time
          }
          else if (count_y == 2 && count_x == 1)
          {
            if (i_result_b.num_available() == 0)
              wait(i_result_b.data_written_event());
            *(target_bitmap + bytes_per_pixel * (width * y + x - 1) + 2) = i_result_r.read();
            *(target_bitmap + bytes_per_pixel * (width * y + x - 1) + 1) = i_result_g.read();
            *(target_bitmap + bytes_per_pixel * (width * y + x - 1) + 0) = i_result_b.read();
            wait(1);
      ...
      // cout << "Now at " << sc_time_stamp() << " TB454451651654244544454545 " << a << endl;
      a += 1;
      // cout << "Now at " << sc_time_stamp() << " TBOUT545asd4as42 " << i << endl;
      // }
    }
